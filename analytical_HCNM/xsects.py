...
class BCM:
    ...
    @staticmethod
    def get_total_xsect(mean_energy_kev, mix_N=0.78, mix_O=0.21, mix_Ar=0.01):
        ...
        if isinstance(energy_ev, np.ndarray):
            ...
            xsect_total = mix_O * O_xsect + mix_N * N_xsect + mix_Ar * Ar_xsect
        else:
            xsect_total = mix_O * BCM.oxygen_xsect(energy_ev) + mix_N * BCM.nitrogen_xsect(energy_ev) \
                + mix_Ar * BCM.argon_xsect(energy_ev)
        return xsect_total
...
def main():
    ...
    plt.plot(E_kev, BCM.get_total_xsect(E_kev))
    plt.show()
    return 0
